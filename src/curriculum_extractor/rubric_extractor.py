...
class RubricExtractor:
    ...
    def parse_rubric_row(self, row_cells: list, col_map: dict[str, int]) -> RubricCriterion | None:
        ...
        exceeding = self._get_cell_text(row_cells[col_map['exceeding']]).strip()
        meeting = self._get_cell_text(row_cells[col_map['meeting']]).strip()
        approaching = self._get_cell_text(row_cells[col_map['approaching']]).strip()
        below = self._get_cell_text(row_cells[col_map['below']]).strip()
        if not all((criterion, exceeding, meeting, approaching, below)):
            return None

        return RubricCriterion(
            criterion=criterion,
            exceeding_expectations=exceeding,
            meeting_expectations=meeting,
            approaching_expectations=approaching,
